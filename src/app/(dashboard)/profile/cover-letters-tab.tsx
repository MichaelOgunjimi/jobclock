...
import { Plus, Trash2, Pencil, X, FileText, Upload, Loader2, AlertTriangle } from "lucide-react"
import { saveLetterTemplate, deleteLetterTemplate } from "./actions"

type Tone = "professional" | "enthusiastic" | "conservative" | "story"
...
const TONES: { value: Tone; label: string; description: string }[] = [
  ...
  { value: "conservative", label: "Conservative", description: "Traditional and measured" },
  { value: "story", label: "Story", description: "Narrative about your journey" },
]
...
function FileUploadZone({ onContent }: { onContent: (text: string) => void }) {
  ...
  const onDrop = useCallback(async (accepted: File[]) => {
    ...
    fd.append("file", file)
    try {
      const res = await fetch("/api/cover-letter/parse", { method: "POST", body: fd })
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{letter.label ?? "Untitled"}</p>
            </div>
          </div>
...
function LetterForm({
  ...
  initial?: LetterTemplate
  onSave: (letter: LetterTemplate) => void
  onCancel: () => void
}) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (result?.error) {
        ...
        onSave(result.letter as LetterTemplate)
      }
    })
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label>Label</Label>
          <Input name="label" defaultValue={initial?.label ?? ""} placeholder="e.g. Software Engineering" required />
        </div>
        <div className="space-y-1.5">
          <Label>Tone</Label>
          <select
      ...
      <div className="space-y-1.5">
        ...
        <textarea
          ...
          rows={10}
          required
          className="flex min-h-[240px] w-full border border-input bg-muted px-4 py-3 text-[13px] tracking-[0.02em] outline-none transition-colors focus:border-ring focus:ring-2 focus:ring-ring/15 resize-none"
        />
      </div>
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Button type="submit" disabled={isPending}>
          {isPending ? "Saving…" : initial ? "Save Changes" : "Add Cover Letter"}
        </Button>
      </div>
    </form>
  )
...
export function CoverLettersTab({ initialLetters }: { initialLetters: LetterTemplate[] }) {
  ...
  function handleSave(letter: LetterTemplate) {
    setLetters((prev) => {
          ...
          <Card>
            <CardContent className="py-14 text-center text-muted-foreground">
              ...
              <p className="text-xs mt-1 opacity-60">
                Add a base cover letter per role type — write it or import from a PDF/DOCX.
              </p>
            </CardContent>
          </Card>
        )}
        ...
        {letters.map((letter) => (
          <Card key={letter.id}>
            <CardContent className="pt-4 pb-4">
              {editingId === letter.id ? (
                <LetterForm initial={letter} onSave={handleSave} onCancel={() => setEditingId(null)} />
              ) : (
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="text-sm font-medium">{letter.label ?? "Untitled"}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{toneLabel(letter.tone)}</p>
                    </div>
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button type="button" onClick={() => setCreating(false)} className="text-muted-foreground hover:text-foreground transition-colors">
                  <X className="h-4 w-4" />
                </button>
              </div>
              <LetterForm onSave={handleSave} onCancel={() => setCreating(false)} />
            </CardContent>
          </Card>
        ) : (
